...
function getSizeStock(product: any, size: string): { total: number; reserved: number; available: number } {
...
export const addToCart = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        // Check per-size stock availability
        for (const sizeItem of sizeItems) {
            const stockInfo = getSizeStock(product, sizeItem.size);
            if (stockInfo.available <= 0) {
                res.status(400).json({ error: `Size ${sizeItem.size} is out of stock` });
                return;
            }
        }
        ...
        for (const sizeItem of sizeItems) {
            ...
            const stockInfo = getSizeStock(product, sizeItem.size);
            if (totalQuantity > stockInfo.available) {
                res.status(400).json({
                    error: `Only ${stockInfo.available} total item(s) available in stock for size ${sizeItem.size}. You already have ${existingItemIndex > -1 ? cart.items[existingItemIndex].quantity : 0} in your cart.`,
                });
                return;
            }
