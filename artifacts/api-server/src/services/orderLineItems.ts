    ...
    for (const raw of rawItems) {
        ...
        if (product) {
            ...
            for (const sizeItem of sizeItems) {
                const total = getCount(sizeCountsMap, sizeItem.size);
                const reserved = getCount(reservedCountsMap, sizeItem.size);
                const available = Math.max(0, total - reserved);
                if (available < sizeItem.quantity) {
                    throw new Error(`Insufficient stock for ${product.name} in size ${sizeItem.size}. Only ${available} available.`);
                }
            }
